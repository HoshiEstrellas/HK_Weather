...
CREATE INDEX IF NOT EXISTS idx_lamppost_type ON lampposts (lp_type);
...
CREATE TABLE IF NOT EXISTS weather_observations (
    ...
    source_processed_at TIMESTAMP NULL,
    source_status VARCHAR(16) NULL,
    version VARCHAR(16) NULL,
    fetched_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    raw_payload CLOB NULL,
    CONSTRAINT uk_weather_source UNIQUE (lp_number, device_id, source_observed_at),
    CONSTRAINT fk_weather_lamppost
        FOREIGN KEY (lp_number) REFERENCES lampposts (lp_number)
        ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_weather_sync_run ON weather_observations (sync_run_id);
CREATE INDEX IF NOT EXISTS idx_weather_lp_time ON weather_observations (lp_number, source_observed_at DESC);
CREATE INDEX IF NOT EXISTS idx_weather_observed_at ON weather_observations (source_observed_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_sync_started_at ON sync_runs (started_at DESC);
