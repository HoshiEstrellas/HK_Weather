...
历史页面只展示当前 10 分钟区间之前 1 小时内获取到的数据，同样按后端获取时间倒序排列。

例如当前时间是 `13:16`，当前区间从 `13:10` 开始，则历史页面查询 `12:10` 到 `13:10` 之间后端获取到的数据。
...
网页端使用纯 HTML/CSS/JavaScript 编写，可以直接调用同一个后端接口。

网页文件位置：

```text
C:\Users\Desktop\weather\web\index.html
```
